...
async function initializeSeoSettings() {
  try {
    ...
    for (const pageData of defaultPages) {
      ...
      if (!existingPage) {
        await PageSeo.create(pageData);
        logger.info(`Created SEO settings for: ${pageData.pagePath}`);
        created++;
      } else {
        logger.info(`SEO settings already exist for: ${pageData.pagePath}`);
        existing++;
      }
    }
    
    logger.info(`SEO initialization complete: ${created} created, ${existing} already existed`);
    console.log(`✅ SEO initialization complete: ${created} created, ${existing} already existed`);
    
    process.exit(0);
  } catch (error) {
    logger.error('Error initializing SEO settings:', error);
