...
export interface IScript extends mongoose.Document {
  ...
  isActive: boolean;
  platform: string; // e.g., 'Google Analytics', 'Facebook Pixel', 'GTM', 'Custom'
  priority: number; // Loading order (lower numbers load first)
  targetPages: string[]; // Specific pages to target, empty array means all public pages
  excludePages: string[]; // Pages to exclude
  createdAt: Date;
  updatedAt: Date;
  createdBy?: mongoose.Types.ObjectId;
  shouldLoadOnPage(pathname: string): boolean;
}
...
ScriptSchema.virtual('safeContent').get(function(this: IScript) {
...
ScriptSchema.methods.shouldLoadOnPage = function(pathname: string): boolean {
  ...
  return this.targetPages.some(targetPage => 
    pathname === targetPage || pathname.startsWith(targetPage + '/')
  );
};

// Static method to get scripts for a specific page and placement
ScriptSchema.statics.getScriptsForPage = function(pathname: string, placement?: string) {
  ...
  return this.find(query)
    ...
    .then((scripts: IScript[]) => 
      scripts.filter(script => script.shouldLoadOnPage(pathname))
    );
};
