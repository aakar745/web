 ...
 */
export async function getCurrentSettings(): Promise<ISystemSettings> {
  const now = Date.now();
  ...
  try {
    ...
    logger.warn('Failed to get settings from database, using environment variables:', error);
 ...
 */
export async function getSetting<K extends keyof ISystemSettings>(key: K): Promise<ISystemSettings[K]> {
...
export async function getLoadBalancerSettings() {
  const settings = await getCurrentSettings();
  return {
