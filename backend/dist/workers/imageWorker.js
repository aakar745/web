 ...
 */
async function sendWebhookNotification(jobId, data, webhookUrl, status, error) {
    try {
        if (!webhookUrl)
            return;
...
redis_1.redisStatusObserver.on('statusChanged', async (isAvailable) => {
    ...
    if (isCheckingRedis || !initialSetupComplete)
        return;
...
async function setupBullWorkers() {
    try {
        ...
        const bullCompressQueue = imageQueue_1.compressQueue;
        const bullResizeQueue = imageQueue_1.resizeQueue;
        const bullConvertQueue = imageQueue_1.convertQueue;
        const bullCropQueue = imageQueue_1.cropQueue;
            ...
            try {
                ...
                bullCompressQueue.process(async (job) => {
                    ...
                    try {
                        ...
                        // Process the image
                        const result = await (0, imageService_1.compressImageService)(filePath, quality);
                        ...
                        // Send webhook notification if URL was provided
                        if (webhookUrl) {
                            await sendWebhookNotification(jobWithId.id || 'unknown', null, webhookUrl, 'failed', error instanceof Error ? error.message : String(error));
                        }
                        throw error;
                    }
                });
            ...
            try {
                ...
                bullResizeQueue.process(async (job) => {
                    ...
                    try {
                        ...
                        // Process the image
                        const result = await (0, imageService_1.resizeImageService)(filePath, width ?? 0, height ?? 0, fit ?? 'contain');
                        // Update progress to 90%
                        await jobWithId.progress(90);
                        // Return output with generated paths for download
                        const filename = path_1.default.basename(result.path);
            ...
            try {
                ...
                bullConvertQueue.process(async (job) => {
                    ...
                    try {
                        ...
                        // Process the image
                        const result = await (0, imageService_1.convertImageService)(filePath, format);
                        // Update progress to 90%
                        await jobWithId.progress(90);
                        // Return output with generated paths for download
                        const filename = path_1.default.basename(result.path);
            ...
            try {
                ...
                bullCropQueue.process(async (job) => {
                    ...
                    try {
                        ...
                        // Process the image
                        const result = await (0, imageService_1.cropImageService)(filePath, left, top, width, height);
                        // Update progress to 90%
                        await jobWithId.progress(90);
                        // Return output with generated paths for download
                        const filename = path_1.default.basename(result.path);
...
function startRedisChecker() {
    ...
    redisCheckerInterval = setInterval(async () => {
        ...
        try {
            ...
            const isRedisAvailable = await (0, redis_1.testRedisConnection)();
