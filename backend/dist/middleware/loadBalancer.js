 ...
 */
async function isSystemUnderHighLoad() {
    try {
        // Get current settings from database
        const { maxLoadThreshold, maxMemoryUsagePercent } = await (0, settingsService_1.getLoadBalancerSettings)();
        // Check CPU load
        ...
        return (cpuLoad > maxLoadThreshold ||
            memoryUsagePercent > maxMemoryUsagePercent ||
            requestPressure);
    }
    catch (error) {
        logger_1.default.error('Error getting load balancer settings, using fallback values:', error);
        ...
        const cpuLoad = os_1.default.loadavg()[0] / os_1.default.cpus().length;
        const totalMemory = os_1.default.totalmem();
        const freeMemory = os_1.default.freemem();
        const memoryUsagePercent = ((totalMemory - freeMemory) / totalMemory) * 100;
        const requestPressure = activeRequests > 200;
...
async function updateDegradationMode() {
    ...
    // Require multiple consecutive high load checks
        ...
        if (now - lastEmergencyCleanup > CLEANUP_COOLDOWN) {
            ...
            logger_1.default.warn('Triggering emergency file cleanup due to high load');
        }
    }
}
/**
...
const loadBalancer = (req, res, next) => {
    ...
    updateDegradationMode().catch(error => {
        logger_1.default.error('Error updating degradation mode:', error);
    });
